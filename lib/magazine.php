...
/**
 * @todo
 * 'thumbnail' - replace with correct image size
 *
 */
...
class N1_Magazine {
    ...
    function set_page_type() {
        ...
        } else if ($wp_query->query['pagename'] == 'online-only') {
      ...
      <section class="post-meta-social">
        ...
        <ul>
          ...
          <li class="post meta social">
            <a title="Twitter" class="social icon twitter" href="<?php echo $tw_href ?>" onclick="return popitup('<?php echo $tw_href ?>')">Twitter</a>
          </li>
          <li class="post meta social">
            <a title="Facebook" class="social icon facebook" href="<?php echo $fb_href ?>" onclick="return popitup('<?php echo $fb_href ?>')">Facebook</a>
          </li>
          <li class="post meta social">
            <a title="Google Plus" class="social icon google" href="<?php echo $g_href ?>" onclick="return popitup('<?php echo $g_href ?>')">Google
              Plus</a></li>
          <li class="post meta social" title="Pocket">
          ...
          <li class="post meta social">
            <a title="Instapaper" class="social icon instapaper" href="<?php echo $i_href ?>" onclick="return popitup('<?php echo $i_href ?>')">Instapaper</a>
          </li>
        </ul>
    ...
    function is_paywalled($post_id = null) {
        ...
        /*    if (self::isAllowedIp(self::get_client_ip(), $ips)) {
                $paywall = false;
            }
        */
        ...
        // member is Free Membership or Web Subscription.
        if (mm_member_decision(array("isMember" => "true", "status" => "active|pending_cancel"))) {
            $paywall = false;
        }
        // If a member is a Gift Sub Giver, paywall is true.
        if (mm_member_decision(array('isMember' => 'true', 'status' => 'active|pending_cancel', 'membershipID' => "1|16"))) {
            $paywall = true;
        }
    ...
    function get_authors($id, $str = true, $linked = true) {
        if (!$authors = wp_get_object_terms($id, 'authors', array('orderby' => 'name', 'order' => 'ASC'))) return false;
        if ($str) {
            $authors_array = array();
                  ...
                  while (the_repeater_field('issue_sections', $toc_post->ID)) {
                    ...
                    <li class="cf">
                      <h3 class="toc-section">
                        ...
                        <?php foreach ($section_posts as $sp) { ?>
                          <div class="toc-article">
                            ...
                            <h4>
                              <a href="<?php echo get_permalink($sp->ID) ?>" title="<?php echo $sp->post_title ?>"><?php echo $sp->post_title ?></a>
                            </h4>
                      ...
                      <li class="entry-content">
                        <img src="http://placehold.it/400x150/e8e8e8/ffffff/&text=Social+Signup" class="aligncenter" alt=""/>
                      </li>
                          <?php
                      }
                      $first_section = false;
