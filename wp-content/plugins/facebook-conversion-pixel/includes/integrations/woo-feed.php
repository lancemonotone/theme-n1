...
function fca_pc_woo_feed_item( $wc_product, $options ) {
	...
	$woo_include_variations = $options['woo_variations'];
	...
	if( $wc_product->is_type( 'variable' ) && $woo_include_variations === 'on' ) {
		forEach ( $wc_product->get_available_variations() as $variation ) {
			...
			?>
			<item>
				<g:id><?php echo fca_pc_encode_xml( $product_id ) . '-' . $variation[ 'variation_id' ] ?></g:id>
				<g:item_group_id><?php echo fca_pc_encode_xml( $product_id ) ?></g:item_group_id>
...
function fca_pc_woo_feed_extra_fields( $wc_product, $attributes = null ) {
	...
	forEach ( $extra_fields as $field ) {
		...
		if ( isSet( $attributes[ 'attribute_pa_' . $field ] ) ) {
			echo "\t<g:$field>" . fca_pc_encode_xml( $attributes[ 'attribute_pa_' . $field ] ) . "</g:$field>\n";
		} else if ( $wc_product->get_attribute( $field ) ) {
			echo "\t<g:$field>" . fca_pc_encode_xml( $wc_product->get_attribute( $field ) ) . "</g:$field>\n";
		}
	}
}
