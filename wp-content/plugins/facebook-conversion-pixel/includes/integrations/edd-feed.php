...
function fca_pc_edd_do_feed_body( $options, $offset = 0 ) {
	...
	forEach( $products as $p ) {
		...
		?>
		<item>
			<g:id><?php echo fca_pc_encode_xml( $p->ID ) ?></g:id>
			<g:title><?php echo fca_pc_encode_xml( get_the_title( $p->ID ) ) ?></g:title>
			<g:description><?php echo $desc_mode === 'content' ? fca_pc_encode_xml( $p->post_content ) : fca_pc_encode_xml( get_the_excerpt( $p->ID ) ) ?></g:description>
			<g:link><?php echo get_permalink( $p->ID ) ?></g:link>
			<g:image_link><?php echo fca_pc_edd_product_image( $p->ID ) ?></g:image_link>
			<g:brand><?php echo fca_pc_encode_xml( bloginfo( 'name' ) ) ?></g:brand>
...
function fca_pc_edd_maybe_add_cat( $id ) {
	$cats = get_the_terms( $id, 'download_category' );
	if ( $cats ) {
		echo "\t\t\t" . '<g:product_type>' .  fca_pc_encode_xml( $cats[0]->name ) . '</g:product_type>';
	}
}
