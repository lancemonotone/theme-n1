...
class swas_wp_footnotes {
	...
	// Display the footnotes (here is where you can change the output)
		...
		// Create each footnote
			$datanote .= '<li id="fn'.$id_num.'-'.$post->ID.'">'; // You can add a class to the list item
			$node = $value['text'].'&nbsp;<a href="#'.$id_id.'"'; // The footnote (don't change this)
			$node .= ' class="backlink" title="Jump back to footnote '.$id_num.' in the text.">'; // You can change the class or hover text
			$node .= '&#8617;'; // The backlink character (↩)... &#8626 (↲) is another common one
			$node .= '</a>';
			$datanote .= wpautop($node).'</li>'; // After the footnote
		}


	// Create the footnotes
		foreach ($footnotes as $key => $value) {
			$data = $data.'<ol class="footnotes"'; // Before the footnotes
