...
{	
	private static function includeLocalFiles($className,$classFileName)
	{
		...
		{
		    $lcfn = strtolower($classFileName);
		    $filetypes = [ "class.{$lcfn}.php", "interface.{$lcfn}.php", "trait.{$lcfn}.php" ];
			
			if(preg_match("/(hooks)/", $className))
			{
			    foreach ($filetypes as $lfn)
			    {
			        $file = MM_PLUGIN_ABSPATH."/hooks/{$lfn}";
			        if(file_exists($file))
			        {
			            require_once($file);
			            return true;
			...
			else
			{
				...
				foreach($dirs as $dir)
				{
				    foreach ($filetypes as $lfn)
				    {
				        $file = MM_PLUGIN_ABSPATH."/{$dir}/{$lfn}";
				        if(file_exists($file))
				        {
				            require_once($file);
				            return true;
