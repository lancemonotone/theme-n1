...
<form name="settings" id="post" method="post" action="admin.php?page=adrotate-settings&tab=stats">
    ...
    <table class="form-table">
        <tr>
            <th><?php _e( 'How to track stats', 'adrotate' ); ?></th>
            <td>
                <select name="adrotate_stats">
                    <option value="0" <?php if ( $adrotate_config[ 'stats' ] == 0 ) {
                        echo 'selected';
                    } ?>><?php _e( 'Disabled - Do not track stats', 'adrotate' ); ?></option>
                    <option value="1" <?php if ( $adrotate_config[ 'stats' ] == 1 ) {
                        echo 'selected';
                    } ?>>AdRotate Statistics (<?php _e( 'Default', 'adrotate' ); ?>)
                    </option>
    ...
    <table class="form-table">
        <tr>
            <th><?php _e( 'Admin stats', 'adrotate' ); ?></th>
            <td>
                <input type="checkbox" name="adrotate_enable_admin_stats" <?php checked( $adrotate_config[ 'enable_admin_stats' ], 'Y' ); ?> />
                <?php _e( 'Track statistics from admin users.', 'adrotate' ); ?>
            </td>
        </tr>
        <tr>
            <th><?php _e( 'Logged in impressions', 'adrotate' ); ?></th>
            <td>
                <input type="checkbox" name="adrotate_enable_loggedin_impressions" <?php checked( $adrotate_config[ 'enable_loggedin_impressions' ], 'Y' ); ?> />
                <?php _e( 'Track impressions from logged in users.', 'adrotate' ); ?>
            </td>
        </tr>
        <tr>
            <th><?php _e( 'Logged in clicks', 'adrotate' ); ?></th>
            <td>
                <input type="checkbox" name="adrotate_enable_loggedin_clicks" <?php checked( $adrotate_config[ 'enable_loggedin_clicks' ], 'Y' ); ?> />
                <?php _e( 'Track clicks from logged in users.', 'adrotate' ); ?>
            </td>
        </tr>
        <tr>
            <th><?php _e( 'Impression timer', 'adrotate' ); ?></th>
            <td>
                <input name="adrotate_impression_timer" type="text" class="search-input" size="5" value="<?php echo $adrotate_config[ 'impression_timer' ]; ?>" autocomplete="off"/> <?php _e( 'Seconds.', 'adrotate' ); ?>
                <br/>
        ...
        <tr>
            <th><?php _e( 'Click timer', 'adrotate' ); ?></th>
            <td>
                <input name="adrotate_click_timer" type="text" class="search-input" size="5" value="<?php echo $adrotate_config[ 'click_timer' ]; ?>" autocomplete="off"/> <?php _e( 'Seconds.', 'adrotate' ); ?>
                <br/>
