...
function limit_login_notify_email($user) {
	...
	if ($whitelisted) {
		$subject = sprintf(__("[%s] Failed sign in attempts from whitelisted IP"
				      , 'limit-login-attempts')
				   , $blogname);
	} else {
		$subject = sprintf(__("[%s] Too many failed sign in attempts"
				      , 'limit-login-attempts')
				   , $blogname);
	}

	$message = sprintf(__("%d failed sign in attempts (%d lockout(s)) from IP: %s"
			      , 'limit-login-attempts') . "\r\n\r\n"
			   , $count, $lockouts, $ip);
	if ($user != '') {
...
function limit_login_error_msg() {
	...
	$msg = __('<strong>Sorry.</strong> That was one too many failed sign in attempts. For everyone&#8217;s safety, we&#8217;ve temporarily frozen your sign in credentials.', 'limit-login-attempts') . ' ';
	...
	if ($when > 60) {
		$when = ceil($when / 60);
		$msg .= sprintf(_n('You should be able to sign in again in %d hour.', 'You should be able to sign in again in  %d hours.', $when, 'limit-login-attempts'), $when);
	} else {
		$msg .= sprintf(_n('You should be able to sign in again in %d minute.', 'You should be able to sign in again in %d minutes.', $when, 'limit-login-attempts'), $when);
	}
