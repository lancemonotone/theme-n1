...
$archive_class = $is_scroll ? 'online-only' : 'archive';
$archive_description = $is_scroll ? get_field('options_online-only_landing_page_dek', 'options') : $current_page->description;
$archive_description = ($current_page->taxonomy == 'authors') ? _('All articles by this author') : $archive_description;
$taxonomy = $is_scroll ? $pagename ?? '' : ($taxonomy ?? ($current_page->taxonomy ?? ''));
$term = $term ?? ($current_page->slug ?? '');
      ...
      <ul>
          <?php foreach (get_terms('online-only') as $cat) { ?>
            <li class="post-meta-term-item">
              <a class="category <?php echo $cat->slug ?>"
                 href="<?php echo get_term_link($cat, 'post') ?>"><?php echo $cat->name ?></a>
            </li>
          <?php } ?>
      </ul>
