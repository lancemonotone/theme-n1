...
<head>
  ...
  gtag('config', 'G-3NRFPFK8X6');
    ...
    <link rel="canonical"
          href="<?php echo home_url() ?><?php echo $_SERVER['REQUEST_URI']; ?>"/>
    <link type="application/opensearchdescription+xml"
          rel="search"
          href="/opensearch.osdd"/>
...
<header id="header-main">
    <div class="nav wrapper">
        <nav class="nav-actions">
            <ul class="nav-actions">
                <li class="nav-actions signin<?php echo is_user_logged_in() ? ' signout' : '' ?>"
                    id="nav-actions-signin">
					<?php wp_loginout( site_url() ) ?><!--Sign In--></li>
                <li class="nav-actions account">
        ...
        <nav class="nav-sections">
            ...
            <div class="nav-sections menu"
                 id="primary-menu">
                <a class="navmenu"
                   href="#">Full Navigation
                </a>
            </div>
        </nav>
    </div>
    ...
    <section
            class="header-signin"
            id="header-signin">
        <h3 class="header-signin category"
            id="header-signin-button">Sign In to n+1</h3>
        <form class="header-signin"
              action="<?php echo home_url() ?>/wp-login.php?redirect_to=<?php echo urlencode( site_url( $_SERVER['REQUEST_URI'] ) ) ?>"
              method="POST">
            <fieldset
                    class="header-signin username">
                <input type="text"
                       id="log"
                       name="log"
                       class="header-signin text username form-text"
                       placeholder="Email"/>
            </fieldset>

            <fieldset
                    class="header-signin password">
                <input type="password"
                       id="pwd"
                       name="pwd"
                       class="header-signin text password form-text"
                       placeholder="Password"/>
            </fieldset>

            <input class="header-signin submit button"
                   type="submit"
        ...
        <div class="header-signin closebox"
             id="header-signin-close">Close
        </div>
    </section>
